...
class Database {
  ...
  beginTransaction() {
    if(!this.transactions.status) this.rollBackIndex.push(0);
    this.transactions.status = true;
    if (this.transactions.status) this.rollBackIndex.push(this.transactions.transactionToRollBack.length);
  }
  ...
  rollbackTransaction() {
    ...
    let index = this.rollBackIndex.pop()
    while(this.transactions.transactionToRollBack.length>index){
